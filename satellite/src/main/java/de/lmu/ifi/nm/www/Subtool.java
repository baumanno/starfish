...
//		private int net_increment;
		...
		public void setSubnetMask(String ip) {
			...
			subnet_addr = Iandr.and(ip_addr, sub_mask);
			broadcast_addr = Iandr.broadcast(subnet_addr, sub_mask);
		...
		public void calculateBitInformation(String sub_mask, char network_class) {
			char[][] bin_sub_mask = Conv.ipToBin(sub_mask);
			int num_bits = 0;
		...
		public void setTotalSubnets(int totalSubnets) {
			total_subnets = Conv.numRound(totalSubnets);
		...
		public void setTotalHosts(int totalHosts) {
			total_hosts = Conv.numRound(totalHosts);
